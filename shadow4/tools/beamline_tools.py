...
def flux_summary(beamline, spectrum_energy=None, spectrum_flux=None, e_min=None, e_max=None, nbins=102):
    ...
    try:
        ...
        if is_monochromatic:
            ...
            try:    initial_flux = trapezoid(spectrum_flux / (1e-3 * spectrum_energy), spectrum_energy)
        ...
        if not is_monochromatic:
            ...
            try:
                txt += " Initial Flux from Source (integrated over histogram): %g ph/s" % (
                    trapezoid(interpolated_flux_per_ev, ticket0['bin_center']))
            except:
                txt += " Initial Flux from Source (integrated over histogram): %g ph/s" % (
            ...
            try:    flux_at_sample_integrated = trapezoid(flux_at_sample, ticket1['bin_center'])
            ...
            try:    power_at_sample_integrated = trapezoid(power_at_sample, ticket1['bin_center'])
