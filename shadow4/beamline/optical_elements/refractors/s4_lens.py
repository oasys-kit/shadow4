...
class S4Lens(Lens, S4LensOpticalElementDecorator):
    ...
    def get_lens_interfaces(self):
        ...
        # get the coefficients that are already stored in syned
        conic_coefficients_all =  self.get_surface_shape1().get_conic_coefficients() # todo:
        # it seems that there is a bug in syned as this should return only the shape1 coeffcients and it returns both shapes.
        # There is also a bug with kw index: 1 should be 0 and 2 should be 1

        conic_coefficients1 = conic_coefficients_all[0]
        conic_coefficients2 = conic_coefficients_all[1]

        # print("????? interface_shapes", interface_shapes, interface_shapes[1].get_coefficients())
